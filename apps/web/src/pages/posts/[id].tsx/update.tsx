...
const Update = () => {
  ...
  const getPost = trpc.post.getPost.useQuery(router.query.id as string);
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    let postUser;
    if (getPost.data) {
      postUser = await updatePost.mutateAsync({
        ...postData,
        id: getPost.data.id,
      });
  ...
  useEffect(() => {
    if (getPost.data && getPost.data.attribute) {
      setPostData({
        ...
        desc: getPost.data.desc || "",
      });
      setAttData({
        id: getPost.data.id || "",
        location: getPost.data.attribute.location || "",
        price: getPost.data.attribute.price || 0,
        size: getPost.data.attribute.size || 0,
        rentStartDate: getPost.data.attribute.rentStartDate || new Date(),
        rentEndDate: getPost.data.attribute.rentEndDate || new Date(),
        furnished: getPost.data.attribute.furnished || false,
        house: getPost.data.attribute.house || false,
        appartment: getPost.data.attribute.appartment || false,
        terrace: getPost.data.attribute.terrace || false,
        pets: getPost.data.attribute.pets || false,
        smoker: getPost.data.attribute.smoker || false,
        disability: getPost.data.attribute.disability || false,
        garden: getPost.data.attribute.garden || false,
        parking: getPost.data.attribute.parking || false,
        elevator: getPost.data.attribute.elevator || false,
        pool: getPost.data.attribute.pool || false,
      });
    }
  }, [getPost.data]);
