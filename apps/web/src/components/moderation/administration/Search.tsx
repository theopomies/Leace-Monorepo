    ...
    <div className="relative">
      <div className="sticky flex w-full items-center justify-between rounded-full bg-white p-2 shadow-lg">
        <input
          ...
          value={uuid}
        />
        <Link href={type === "none" ? "#" : `/administration/${type}s/${uuid}`}>
          <div
            className={`mx-2 cursor-pointer rounded-full bg-gray-600 p-2 ${
              type === "none" ? "" : "hover:bg-blue-400"
            }`}
          >
            {!isLoading && (
              <svg
            ...
            {isLoading && (
              <svg
                aria-hidden="true"
                className="mr-2 h-4 w-4 animate-spin fill-blue-300 text-gray-200 dark:text-gray-600"
                viewBox="0 0 100 101"
                fill="none"
              >
                <path
